...
{
    int other = 1 - self;
    atomic_store_explicit(&flag[self], 1, memory_order_release);
    atomic_store_explicit(&turn, other, memory_order_relaxed);
...
{
    atomic_store_explicit(&flag[self], 0, memory_order_release);
}
...
int main(int argc, char *argv[]) {
    ...
    FILE *f_actual = fopen("p_weak.csv", "a");
    if (f_actual == NULL) {
    ...
    FILE *f_time = fopen("p_weak_time.csv", "a");
    if (f_time == NULL) {
